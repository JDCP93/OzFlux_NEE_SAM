...
parJagsModel(cl, name = 'par_nee_model', file = NEEModel_DIC, data = `SturtPlains_Input`,
             n.chains = 6, n.adapt = 5000, quiet=FALSE)
message("Update the model at ",Sys.time())
parUpdate(cl, "par_nee_model", n.iter = 5000)
...
save(nee_daily, file=paste('NEE_output_site_SP_DIC', Sys.Date(),'.rda', sep = ''))
  

rm(nee_daily)

detach(`SturtPlains_Input`)
stopCluster(cl)
