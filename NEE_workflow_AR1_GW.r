...
load('./GreatWesternWoodlands_Input_NDVI.Rdata') # change these to functions
attach(`GreatWesternWoodlands_Input`)


# Load Howard Springs data
load('results/NEE_output_site_GW_2020-08-27.rda')
# data inside is called "nee_daily"
assign('GW',nee_daily)
rm(nee_daily)
summary=summary(GW)
NEE_pred = summary$statistics[substr(rownames(summary$statistics),1,3)=="NEE",1]
NEE_obs = `GreatWesternWoodlands_Input`$NEE[-(1:365)]

NEE.res = NEE_pred - NEE_obs
Nmem = GreatWesternWoodlands_Input$Nmem
...
save(AR1.res, file=paste('NEE_AR1_output_site_GW_', Sys.Date(),'.rda', sep = ''))
  


detach(`GreatWesternWoodlands_Input`)
stopCluster(cl)
