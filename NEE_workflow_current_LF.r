...
load('./Litchfield_Input_NDVI.Rdata') # change these to functions
attach(`Litchfield_Input`)
...
parJagsModel(cl, name = 'par_nee_model', file = NEEModel_current, data = `Litchfield_Input`,
...
save(nee_daily, file=paste('NEE_current_output_site_LF_', Sys.Date(),'.rda', sep = ''))
  

rm(nee_daily)

detach(`Litchfield_Input`)
stopCluster(cl)
