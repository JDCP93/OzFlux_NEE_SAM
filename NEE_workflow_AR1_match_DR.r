...
load('./DryRiver_Input_2013-01-01_2017-12-31.Rdata') # change these to functions
attach(`DryRiver_Input`)


# Load Howard Springs data
load('results/NEE_output_match_site_DR_2020-08-12.rda')
# data inside is called "nee_daily"
assign('DR',nee_daily)
rm(nee_daily)
summary=summary(DU)
NEE_pred = summary$statistics[substr(rownames(summary$statistics),1,3)=="NEE",1]
NEE_obs = `DryRiver_Input`$NEE[-(1:365)]

NEE.res = NEE_pred - NEE_obs
Nmem = DryRiver_Input$Nmem
...
save(AR1.res, file=paste('NEE_AR1_output_match_site_DR_', Sys.Date(),'.rda', sep = ''))
  


detach(`DryRiver_Input`)
stopCluster(cl)
