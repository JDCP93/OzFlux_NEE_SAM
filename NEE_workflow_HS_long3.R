...
load('./HowardSprings_Input_NDVI.Rdata') # change these to functions
attach(`HowardSprings_Input`)
...
parJagsModel(cl, name = 'par_nee_model', file = NEEModel, data = `HowardSprings_Input`,
             n.chains = 6, n.adapt = 25000, quiet=FALSE)
message("Update the model at ",Sys.time())
parUpdate(cl, "par_nee_model", n.iter=25000)


samp_iter <- 250000
message("Start the coda sampling at ",Sys.time())
nee_daily <- parCodaSamples(cl, "par_nee_model", variable.names = nee_monitor_vars, n.iter = samp_iter, thin = 100)
message("Save model output at ",Sys.time())
save(nee_daily, file=paste('NEE_output_long3_site_HS_', Sys.Date(),'.rda', sep = ''))
  

rm(nee_daily)

detach(`HowardSprings_Input`)
stopCluster(cl)
