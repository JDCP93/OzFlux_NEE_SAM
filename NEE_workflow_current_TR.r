...
load('./Tumbarumba_Input_NDVI.Rdata') # change these to functions
attach(`Tumbarumba_Input`)
...
parJagsModel(cl, name = 'par_nee_model', file = NEEModel_current, data = `Tumbarumba_Input`,
...
save(nee_daily, file=paste('NEE_current_output_site_TR_', Sys.Date(),'.rda', sep = ''))
  

rm(nee_daily)

detach(`Tumbarumba_Input`)
stopCluster(cl)
