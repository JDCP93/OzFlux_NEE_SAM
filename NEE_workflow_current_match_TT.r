...
load('./TiTreeEast_Input_2013-01-01_2017-12-31.Rdata') # change these to functions
attach(`TiTreeEast_Input`)
...
parJagsModel(cl, name = 'par_nee_model', file = NEEModel_current, data = `TiTreeEast_Input`,
...
save(nee_daily, file=paste('NEE_current_output_match_site_TT_', Sys.Date(),'.rda', sep = ''))
  

rm(nee_daily)

detach(`TiTreeEast_Input`)
stopCluster(cl)
