WeightPlot_RTPVS = function(Sites,Vars = c("Tair","Fsd","VPD","curSWC","antSWC","Precip","SWC")){
  ...
  CumWeights = data.frame("Site"=rep(Sites,each = 78),
                    "Variable" = rep(rownames(eval(as.name(Sites[1]))$CumWeights),length(Sites)),
                    "Lag" = rep(0,each =78*length(Sites)),
  ...
  CumWeights$Variable[substr(CumWeights$Variable,13,13)==4] = "curSWC"
  CumWeights$Variable[substr(CumWeights$Variable,13,13)==5] = "antSWC"
  CumWeights$Variable[substr(CumWeights$Variable,11,12)=="AP"] = "Precip"
