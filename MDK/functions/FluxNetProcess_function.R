FluxNetProcess = function(Site){
  ...
  for (i in 366:nrow(Data)){
    ppt[i,1] = sum(Data$P_F[(i-1):(i-14)],na.rm=TRUE)
    ppt[i,2] = sum(Data$P_F[(i-15):(i-30)],na.rm=TRUE)
    ppt[i,3] = sum(Data$P_F[(i-31):(i-60)],na.rm=TRUE)
    ppt[i,4] = sum(Data$P_F[(i-61):(i-120)],na.rm=TRUE)
    ppt[i,5] = sum(Data$P_F[(i-121):(i-180)],na.rm=TRUE)
    ppt[i,6] = sum(Data$P_F[(i-181):(i-270)],na.rm=TRUE)
    ppt[i,7] = sum(Data$P_F[(i-271):(i-365)],na.rm=TRUE)
    ppt[i,8] = sum(Data$P_F[(i-1):(i-30)],na.rm=TRUE)
    ppt[i,9] = sum(Data$P_F[(i-31):(i-90)],na.rm=TRUE)
