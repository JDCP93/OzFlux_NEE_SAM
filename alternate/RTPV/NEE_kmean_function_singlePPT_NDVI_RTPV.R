...
NEE_singlePPT_NDVI_kmean_RTPV = function(Site,k,PPTLag = 2){
...
if (PPTLag %in% c(2,3,4,5,6,7,8)){
  message("Performing ",k,"-cluster k-means clustering with NDVI and precip ",PPTLagDef[PPTLag-1]," for ",Site," at ",Sys.time())
} else {
...
climate = cbind(input$clim,input$NDVI,input$ppt_multiscale[,PPTLag])
...
save(output,file = paste0("alternate/RTPV/results/NEE_output_",k,"cluster_kmean_PPT",PPTLagName[PPTLag-1],"_NDVI_RTPV_",Site,".Rdata"))
