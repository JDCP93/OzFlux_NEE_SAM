...
NEE_current_kmean_RTPV = function(Site,k){
...
message("Performing ",k,"-cluster k-means clustering with current climate for ",Site, " at ",Sys.time())
...
output[["r.squared"]] = summary(lm(NEE_obs ~ NEE_pred))$r.squared
...
save(output,file = paste0("alternate/RTPV/results/NEE_output_",k,"cluster_kmean_current_RTPV_",Site,".Rdata"))
