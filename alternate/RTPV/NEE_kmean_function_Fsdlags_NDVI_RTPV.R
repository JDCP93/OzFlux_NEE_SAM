...
NEE_Fsdlags_NDVI_kmean_RTPV = function(Site,k){
...
message("Performing ",k,"-cluster k-means clustering with NDVI and all Fsd lags for ",Site," at ",Sys.time())
...
climate = cbind(input$clim,
                c(NA,input$clim[-climend,2]),
                c(rep(NA,2),input$clim[-((climend-1):climend),2]),
                c(rep(NA,3),input$clim[-((climend-2):climend),2]),
                c(rep(NA,4),input$clim[-((climend-3):climend),2]),
                c(rep(NA,5),input$clim[-((climend-4):climend),2]),
                c(rep(NA,6),input$clim[-((climend-5):climend),2]),
                c(rep(NA,7),input$clim[-((climend-6):climend),2]),
                c(rep(NA,8),input$clim[-((climend-7):climend),2]),
                c(rep(NA,9),input$clim[-((climend-8):climend),2]),
                c(rep(NA,10),input$clim[-((climend-9):climend),2]),
                c(rep(NA,11),input$clim[-((climend-10):climend),2]),
                c(rep(NA,12),input$clim[-((climend-11):climend),2]),
                c(rep(NA,13),input$clim[-((climend-12):climend),2]),
                input$NDVI)
...
save(output,file = paste0("alternate/RTPV/results/NEE_output_",k,"cluster_kmean_Fsdlags_NDVI_RTPV_",Site,".Rdata"))
