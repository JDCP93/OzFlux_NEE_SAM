...
load('inputs/RTPV/AU-Wom_Input_RTPV.Rdata') 
data = `AU-Wom_Input`

# Load in the output data 
# Look in folder "results" for the data
File = list.files("output/RTPV/",pattern = "NEE_output_RTPV_AU-Wom")
...
output = list("output.mcmc"=output.mcmc,
              ...
              "pD" = pD)
# Save the results
save(output, file=paste('NEE_AR1_output_RTPV_AU-Wom_', Sys.Date(),'.Rdata', sep = ''))
