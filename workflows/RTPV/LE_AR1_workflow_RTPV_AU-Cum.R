...
load('inputs/RTPV/LE/AU-Cum_input_LE_RTPV.Rdata') 
data = `AU-Cum_input`

# Load in the output data 
# Look in folder "results" for the data
File = list.files("output/RTPV/",pattern = "LE_output_RTPV_AU-Cum")
...
output = list("output.mcmc"=output.mcmc,
              ...
              "pD" = pD)
# Save the results
save(output, file=paste('LE_AR1_output_RTPV_AU-Cum_', Sys.Date(),'.Rdata', sep = ''))
