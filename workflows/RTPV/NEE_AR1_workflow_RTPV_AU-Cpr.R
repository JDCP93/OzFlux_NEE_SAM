...
load('inputs/RTPV/AU-Cpr_Input_RTPV.Rdata') 
data = `AU-Cpr_Input`

# Load in the output data 
# Look in folder "results" for the data
File = list.files("output/RTPV/",pattern = "NEE_output_RTPV_AU-Cpr")
...
output = list("output.mcmc"=output.mcmc,
              ...
              "pD" = pD)
# Save the results
save(output, file=paste('NEE_AR1_output_RTPV_AU-Cpr_', Sys.Date(),'.Rdata', sep = ''))
