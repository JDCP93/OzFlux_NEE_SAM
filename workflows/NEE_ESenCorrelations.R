...
colnames = c("Metric",sprintf("%s%s%s",rep(Vars,each=2,times=3),rep(c("","NATT","SAWS"),each=10),rep(c("Val","P"),times=15)))
# Initialise the dataframe for the correlation values and p values
Correlations = data.frame(matrix(nrow=19,ncol=31))
colnames(Correlations) = colnames
Correlations["Metric"] = colnames(WorldClimMetrics[-(1:4)])
...
for (i in 5:ncol(WorldClimMetrics)){
  for (Var in Vars){
    ...
    message(Var,
            ...
            " and p value ",
            round(P,3))
    Correlations[i-4,paste0(Var,"Val")] = R2
    Correlations[i-4,paste0(Var,"P")] = P
    ...
    message(Var,
            ...
            " and p value ",
            round(P,3))
    Correlations[i-4,paste0(Var,"NATTVal")] = R2
    Correlations[i-4,paste0(Var,"NATTP")] = P
  ...
  # Calculate correlations for just SAWS sites
    ...
    message(Var,
            ...
            " and p value ",
            round(P,3))
    Correlations[i-4,paste0(Var,"SAWSVal")] = R2
    Correlations[i-4,paste0(Var,"SAWSP")] = P
  }
}

Correlations_longer = pivot_longer(Correlations,names_to = "Measure",values_to = "Values",cols = 2:31)
