...
load('./AU-Tum_Input.Rdata') 
data = `AU-Tum_Input`

# Load in the output data 
# Look in folder "results" for the data
File = list.files("results",pattern = "NEE_output_AU-Tum")
...
output <- jags.parallel(model.file = 'NEEModel_AR1_v2.R',
...
output = list("output.mcmc"=output.mcmc,
              ...
              "pD" = pD)
# Save the results
save(output, file=paste('NEE_AR1_output_AU-Tum_', Sys.Date(),'.Rdata', sep = ''))
